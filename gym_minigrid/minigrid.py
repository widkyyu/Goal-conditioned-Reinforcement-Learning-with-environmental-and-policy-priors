...
class MiniGridEnv(gym.Env):
    ...
    def __init__(
        ...
        agent_view_size: int = 17,  #默认值为7
        render_mode: Optional[str] = None,
        highlight: bool = False,  #True为显示agent的部分可观区域
        ...
        **kwargs,
        ...
        self.carrying = None
        self.agent_coordinate = None
        self.agent_coordinate_display = None
    ...
    def step(self, action):
        ...
        if action == self.actions.left:
            ...
            if left_cell is None or left_cell.can_overlap():
                self.agent_pos = tuple(left_pos)
            if left_cell is not None and left_cell.type == "goal":
                terminated = True
                reward = self._reward()
        ...
        #  right
            ...
            if right_cell is None or right_cell.can_overlap():
                self.agent_pos = tuple(right_pos)
            if right_cell is not None and right_cell.type == "goal":
                terminated = True
                reward = self._reward()
        
        # up
        elif action == self.actions.up:
            up_pos = (self.agent_pos[0],self.agent_pos[1]-1)
            up_cell = self.grid.get(*up_pos)
            if up_cell is None or up_cell.can_overlap():
                self.agent_pos = tuple(up_pos)
            if up_cell is not None and up_cell.type == "goal":
                terminated = True
                reward = self._reward()

        #  down
        elif action == self.actions.down:
            down_pos = (self.agent_pos[0],self.agent_pos[1]+1)
            down_cell = self.grid.get(*down_pos)
            if down_cell is None or down_cell.can_overlap():
                self.agent_pos = tuple(down_pos)
            if down_cell is not None and down_cell.type == "goal":
                terminated = True
                reward = self._reward()
        
        #  done
        elif action == self.actions.done:
            down_pos = (self.agent_pos[0],self.agent_pos[1])
            down_cell = self.grid.get(*down_pos)
            if down_cell is None or down_cell.can_overlap():
                self.agent_pos = tuple(down_pos)
            if down_cell is not None and down_cell.type == "goal":
        ...
        # Move forward
            ...
            if fwd_cell is not None and fwd_cell.type == "goal":
                terminated = True
                reward = self._reward()
            if fwd_cell is not None and fwd_cell.type == "subgoal":
                terminated = True
                reward = self._reward()
    ...
    def gen_obs_grid(self, agent_view_size=None):
        ...
        topX, topY, botX, botY = self.get_view_exts(agent_view_size) #获取agent的视野范围
    ...
    def get_full_render(self):
        ...
        self.agent_coordinate = highlight_mask
        # Render the whole grid
