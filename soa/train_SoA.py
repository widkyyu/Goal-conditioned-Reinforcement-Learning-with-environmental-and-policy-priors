...
if __name__ == "__main__":
    ...
    parser.add_argument("--seed",type=int,help="random seed to generate the environment with",default=221)
    ...
    agent.heatmapfilename = 'SoA'+'_'+ args.env +'_'+ str(seed) + 'seed_' + datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
    agent.future3positionfilename = args.fp_file + '/soa_' + datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
    agent.name = 'soa'+'_'+args.env+'_'+str(seed) +'seed_' 
    ...
    for i_ep in tqdm(range(args.num_episodes)):#range(args.num_episodes)
        ...
        for t in range(10000):  #range(10000)
            ...
            if buffer.pre_full:
                ...
                print('update')
                agent.update_policy(buffer.pre_buffer,device,i_ep)
                agent.update_orientation(buffer_fp_new, device,i_ep)
